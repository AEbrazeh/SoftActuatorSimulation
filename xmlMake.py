...
def createSegment(name, nSegments, L, r, N, innerStiffness, innerDamping, outerStiffness, outerDamping, nSprings, gear, mass = 0.1, dt = 0.01, percentage = 0.5):
    ratio = 2 / percentage - 2
    dx = L/(nSegments * (N * (ratio + 2) - ratio))
    ...
    #ET.SubElement(part, 'joint', pos = '0 0 0', type='hinge', axis='0 1 0')
    #ET.SubElement(part, 'joint', pos = '0 0 0', type='hinge', axis='0 0 1')
    ...
    for ii in range(nSprings):
        ET.SubElement(part, 'site', name = 'A{:02d}B{:02d}S{:02d}C'.format(0, 0, ii+1), size = '0.0001', pos = '{} {} {}'.format(0, r * np.cos(2 * np.pi * ii / nSprings).round(12), r * np.sin(2 * np.pi * ii / nSprings).round(12)))
    for kk in range(nSegments):
        for jj in range(N):
            if kk == 0 and jj == 0: continue
            if jj == 0:
                part = ET.SubElement(part, 'body', name = 'A{:02d}B{:02d}'.format(kk, jj), pos = '{} 0 0'.format(0))
            else:
                part = ET.SubElement(part, 'body', name = 'A{:02d}B{:02d}'.format(kk, jj), pos = '{} 0 0'.format((ratio + 2) * dx))
            ...
            for ii in range(nSprings):
                ET.SubElement(part, 'site', name = 'A{:02d}B{:02d}S{:02d}C'.format(kk, jj, ii+1), size = '0.0001', pos = '{} {} {}'.format(0, r * np.cos(2 * np.pi * ii / nSprings).round(12), r * np.sin(2 * np.pi * ii / nSprings).round(12)))      
            if jj != 0:
    ...
    for kk in range(nSegments):
        for ii in range(1, nSprings + 1):
            for jj in range(N - 1):
                sc = ET.SubElement(tend, 'spatial', name='A{:02d}B{:02d}T{:02d}'.format(kk, jj, ii), stiffness='{}'.format(innerStiffness * (N - 1)), damping='{}'.format(innerDamping * (N - 1)), width='0.001', rgba='0.8 0.0 0.0 1')
    ...
    for kk in range(nSegments):
        for ii in range(1, nSprings + 1):
            for jj in range(N - 1):
                sc = ET.SubElement(tend, 'spatial', name='A{:02d}B{:02d}T{:02d}_'.format(kk, jj, ii), stiffness='{}'.format(outerStiffness * (N - 1)), damping='{}'.format(outerDamping * (N - 1)), width='0.001', rgba='0.8 0.0 0.0 1')
                ET.SubElement(sc, 'site', site='A{:02d}B{:02d}S{:02d}C'.format(kk, jj, ii))
                ET.SubElement(sc, 'site', site='A{:02d}B{:02d}S{:02d}C'.format(kk, jj+1, ii))

    for kk in range(nSegments):
        sc = ET.SubElement(tend, 'spatial', name='T{}'.format(kk), stiffness='0', damping='0', width='0.005', rgba='{} {} {} 1'.format(0.8 * (1 - (kk + 1) / nSegments), 0, 0.8 * (kk + 1) / nSegments))
        for jj in range(N):
    ...
    for kk in range(nSegments):
        ET.SubElement(act, 'general', tendon='T{}'.format(kk), ctrllimited='true', ctrlrange='0 1', gear='{}'.format(gear))
